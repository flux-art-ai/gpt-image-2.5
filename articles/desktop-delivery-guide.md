...
截至 2026 年 9 月 10 日，Flux Art 专题页展示低、中、高、xhigh、max、Auto，以及 1K、2K、4K 和自定义尺寸。社媒预览、小样确认和印刷交付的需求不同。先用较低成本规格确定构图，最终稿再按实际像素要求提高质量，通常比所有尝试都用最高档更可控。

Auto 会根据任务调度资源，固定档位更利于重复比较；无论选择哪种，都应在提交前查看页面当前的费用与说明。完整判断可参考 [质量与尺寸指南](https://gptimagezh.com/faq/gpt-image-2-5-quality-size/)。
...
不需要。Flux Art 提供网页生成和编辑界面，使用更新后的常见浏览器即可。

### 电脑端为什么更适合正式交付？

大屏幕更便于放大检查细节、并排比较版本、核对文件名与像素，也更容易整理参考图、提示词和验收记录。

### 可以同时开很多标签批量提交吗？

不建议用多标签无序提交替代任务管理。先确认一张小样和命名规则，再按页面任务状态有序处理，避免重复生成。

### 下载后应该保存哪些信息？

至少保留原始输入、提示词、模型版本、质量尺寸、生成日期、结果文件和验收结论；涉及客户项目时按团队规则归档。

### 电脑端应该使用哪一种浏览器？

使用仍在更新且能正常运行现代网页的常见浏览器即可，具体兼容性以官网当前说明为准。出现问题时记录浏览器版本，并在另一浏览器复测。

### 怎样公平比较 Flare 和 Sunburst？

保持输入图、提示词、比例和质量设置一致，只改变版本，再比较指令遵循、文字、边缘、构图和耗时。不要用两个不同任务的结果直接下结论。

## FAQ

**Q: 参考图很多时文件夹怎么整理？**

按“主体、风格、构图、文字、禁改项”分类，并在文件名中写明用途和版本。输入职责清楚，后续更容易判断是哪张参考图造成冲突。

**Q: 下载图片看起来模糊，先检查什么？**

先打开真实下载文件，核对像素和文件大小，不要用浏览器缩略图判断。再检查生成规格、下载是否完成以及后续软件是否压缩。

**Q: 电脑屏幕上的颜色能当作商品标准色吗？**

不能。模型结果和未经校准的屏幕都不能替代色卡、实物或受控色彩流程，涉及商品颜色时必须与原始资料核对。

**Q: 包装文字和 Logo 应该怎样验收？**

用 100% 比例查看，并与原图或批准稿逐字、逐位置比较。发现一处修改后，还要重新检查整张图，避免局部返修引入新错误。
...
至少保存原始输入、提示词、参考图、模型版本、质量尺寸、下载结果和验收结论。团队项目还应记录负责人、日期和可交付版本。

价格、账号权益和动态选项以官网当前为准。

## 继续阅读

- [参考图编辑](../docs/reference-editing.md)
- [GPT Image 2.5 国内手机使用教程](mobile-browser-guide.md)
